...
const Address = require('./dist/module_path');
...
class CHSCDN{
    constructor(){
        ...
        this.error_log = jsonfile.readFileSync(new Address().log_path());
    ...
    return new Promise((resolve, reject) => {
        if(!this.isValidImage(filePath)){
            console.error(
                ...
                `${new URL('https://chsweb.vercel.app/docs?search=Media%20Format')}\n\n`
            );
            return reject(false);
        }
    ...
    async function sendPart(part, index, limit, url, key){
        ...
        while(attempts < 3){
            ...
            try{
                ...
                console.log(`Error on attempt ${attempts} for part ${index}:`, error.message, attempts>=3?'\n':'');
            }
        }
        return false;
    }
